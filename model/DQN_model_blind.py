...
class Agent():
    def __init__(self, env, device, hidden_size=64, speed_IG=2, pretrained_state_dict=None):
    ...
    def plot_scores(self):
        ...
        plt.legend(['Score', '100-episode average'], loc='upper left')
        plt.plot()
        # plt.ylim(ymin=0)
        plt.ioff()
            ...
            for t in count():
                ...
                if done:
                    ...
                    if self.env.score < self.env.best_score:
                        ...
                        try:
                            time.sleep(0.5)
                            loc = pyautogui.locateOnScreen('img/save2.png', confidence=0.8)
                            pyautogui.click(pyautogui.center(loc))
                            time.sleep(0.5)
                        except Exception as e:
                            self.env.logger.error(f"Error saving replay: {e}")
                            pydirectinput.press('escape')
                    if self.env.score < self.env.max_race_time:
                        for _ in range(3):
                            time.sleep(0.01)
                            pydirectinput.press('enter')
                    else:
                        race_time = self.env.server_get_info()['race_time'] / 1000
                        if race_time > self.env.max_race_time:
                            for _ in range(3):
                                time.sleep(0.01)
                                pydirectinput.press('enter')
                    break
